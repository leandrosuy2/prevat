        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h3 class="card-title fs-14">{{$professional ? 'Edição de ' : 'Cadastro do novo '}} Profissional</h3>
                    <button wire:click="addInput()" class="fw-semibold btn btn-sm btn-warning"><i class="fe fe-plus-circle"></i> Adicionar formação </button>
                </div>
                <div class="card-body">
                    ...
                    <form wire:submit="save" class="form-horizontal">
                        <div class="row">
                            ...
                            <div class="col-md-12 col-xl-4">
                                <div class="form-group">
                                    ...
                                    <input t wire:model="state.email" class="form-control">
                                    @error('state.email')
                                    <p class="text-danger">{{ $message }}</p>
                                    @enderror
                                </div>
                        ...
                        <p class="card-sub-title text-muted">
                            Escolha as formações do profissional.
                        </p>
                        <div class="row">
                            @foreach($formations as $key => $formation)
                                <div class="col-md-12 col-xl-6">
                                    <div class="form-group">
                                        <label for="Nome do Usuário" class="form-label">Formação</label>
                                        <div class="row gutters-xs">
                                            <div class="col">
                                                <select wire:model="formations.{{$key}}.qualification_id" class="form-control form-select "
                                                        data-placeholder="Escolha">
                                                    @foreach($response->qualificatons as $itemQualification)
                                                        <option value="{{$itemQualification['value']}}" @if(isset($user) && $user['role_id'] == $itemQualification['value']) selected @endif>
                                                            {{$itemQualification['label']}}</option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <span class="col-auto">
                                                @if(isset($formation['id']) && $formation['id'])
                                                    <button wire:click.prevent="removeInput({{$key}}, '{{$formation['id']}}')" class="btn btn-icon btn-danger"  >
                                                        <i class="fe fe-minus-circle"></i>
                                                    </button>
                                                @elseif ($formations[$key])
                                                    <button  wire:click.prevent="removeInput({{$key}})" class="btn btn-icon btn-danger"  >
                                                        <i class="fe fe-minus-circle"></i>
                                                    </button>
                                                @endif
                                            </span>
