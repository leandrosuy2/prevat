                ...
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <div class="">
                        @if(!$participantTraining)
                            ...
                            @if(isset($state['company_id']) && $state['company_id'] != null && isset($state['training_id']) && $state['training_id'] != null && isset($state['contract_id']) && $state['contract_id'] != null)
                                <button class="btn btn-sm btn-green" type="button" wire:click="openSlide('movement.participant-training.private.participants.card', {'id' : '{{$state['company_id']}}', 'contract_id' : {{$state['contract_id']}} })">
                                    <i class="fa fa-user"></i> Adicionar Participantes
                                </button>

                                <button class="btn btn-sm btn-primary" type="button" wire:click="openModal('movement.participant-training.private.participants.form', {'id' : '{{$state['company_id']}}', 'contract_id' : {{$state['contract_id']}} })">
                                    <i class="fa fa-user-plus"></i> Cadastrar Participante
                                </button>
                            @endif
                        ...
                        <div class="row">
                            <div class="col-md-12 col-xl-5">
                                <div class="form-group" >
                                    ...
                                    <x-select2 wire:model.live="state.training_id" class="form-select select2 select2-show-search">
                                        @foreach($response->trainings as $itemTraining)--}}
                                        <option value="{{$itemTraining['value']}}" @if(isset($schedulePrevat) && $schedulePrevat['training_id'] == $itemTraining['value']) selected @endif>
                                            {{$itemTraining['label']}}</option>
                                        @endforeach
                                    </x-select2>
                                    @error('state.training_id')
                            ...
                            <div class="col-md-12 col-xl-5">
                                <div class="form-group" >
                                    ...
                                    <x-select2 wire:model.live="state.company_id" class="form-select select2 select2-show-search">
                                        @foreach($response->companies as $itemCompany)--}}
                                        <option value="{{$itemCompany['value']}}" @if(isset($schedulePrevat) && $schedulePrevat['training_id'] == $itemCompany['value']) selected @endif>
                                            {{$itemCompany['label']}}</option>
                                        @endforeach
                                    </x-select2>
                                    @error('state.company_id')
                                    <p class="text-danger">{{ $message }}</p>
                                    @enderror
                                </div>
                            </div>
                            ...
                            <div class="col-md-12 col-xl-3">
                                <div class="form-group">
                                    ...
                                    <select wire:model="state.training_local_id" class="form-control form-select">
                                        ...
                                        @if($response->participants)
                                            @foreach($response->participants as $key => $itemParticipant)
                                                <tr class="{{$itemParticipant['table']}}">
                                                    ...
                                                    <td class="text-nowrap align-middle fs-11">
                                                        <span>{{ mb_strimwidth($itemParticipant['company'], 0,22,"...")}}</span>
                                                    </td>
                                                    <td class="text-nowrap align-middle fs-11">
                                                        <input min="0" max="10" wire:model.live="quantity.{{$key}}" class="form-control @error('quantity.*') is-invalid state-invalid @enderror" >
                                                    </td>
